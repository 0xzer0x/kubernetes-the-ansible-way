...
check_cert_and_key() {
	...
	if [ -z $cert -o -z $key ]; then
		...
		if [ $CERT_SUBJECT == "${subject}" ] && [ $CERT_ISSUER == "${issuer}" ] && [ $CERT_MD5 == $KEY_MD5 ]; then
...
check_cert_only() {
	...
	if [ -z $cert ]; then
		...
		if [[ $cert == *kubelet-client-current* ]]; then
...
check_cert_adminkubeconfig() {
	if [ -z $ADMINKUBECONFIG ]; then
		...
		if [ $ADMINKUBECONFIG_SUBJECT == "Subject:CN=admin,O=system:masters" ] && [ $ADMINKUBECONFIG_ISSUER == "Issuer:CN=KUBERNETES-CA,O=Kubernetes" ] && [ $ADMINKUBECONFIG_CERT_MD5 == $ADMINKUBECONFIG_KEY_MD5 ] && [ $ADMINKUBECONFIG_SERVER == "https://127.0.0.1:6443" ]; then
...
check_systemd_etcd() {
	if [ -z $ETCDCERT ] && [ -z $ETCDKEY ]; then
		...
		if [ $CERT_FILE == $ETCDCERT ] && [ $KEY_FILE == $ETCDKEY ] && [ $PEER_CERT_FILE == $ETCDCERT ] && [ $PEER_KEY_FILE == $ETCDKEY ] &&
			[ $TRUSTED_CA_FILE == $ETCD_CA_CERT ] && [ $PEER_TRUSTED_CA_FILE = $ETCD_CA_CERT ]; then
		...
		if [ $IAP_URL == "https://$PRIMARY_IP:2380" ] && [ $LP_URL == "https://$PRIMARY_IP:2380" ] && [ $LC_URL == "https://$PRIMARY_IP:2379,https://127.0.0.1:2379" ] &&
			[ $AC_URL == "https://$PRIMARY_IP:2379" ]; then
...
check_systemd_api() {
	if [ -z $APICERT ] && [ -z $APIKEY ]; then
		...
		if [ $ADVERTISE_ADDRESS == $PRIMARY_IP ] && [ $CLIENT_CA_FILE == $CACERT ] && [ $ETCD_CA_FILE == $CACERT ] &&
			...
			[ $SERVICE_ACCOUNT_KEY_FILE == $SACERT ] && [ $TLS_CERT_FILE == $APICERT ] && [ $TLS_PRIVATE_KEY_FILE == $APIKEY ]; then
...
check_systemd_kcm() {
	...
	if [ -z $KCMCERT ] && [ -z $KCMKEY ]; then
		printf "${FAILED}please specify cert and key location\n${NC}"
		exit 1
	elif [ -f $SYSTEMD_KCM_FILE ]; then
		...
		if [ $CLUSTER_SIGNING_CERT_FILE == $CACERT ] && [ $CLUSTER_SIGNING_KEY_FILE == $CAKEY ] && [ $KUBECONFIG == $KCMKUBECONFIG ] &&
			[ $ROOT_CA_FILE == $CACERT ] && [ $SERVICE_ACCOUNT_PRIVATE_KEY_FILE == $SAKEY ]; then
...
check_systemd_ks() {
	...
	if [ -z $KSCERT ] && [ -z $KSKEY ]; then
		printf "${FAILED}please specify cert and key location\n${NC}"
		exit 1
	elif [ -f $SYSTEMD_KS_FILE ]; then
		...
		if [ $KUBECONFIG == $KSKUBECONFIG ]; then
...
1)
	if ! [ "${HOST}" = "controlplane01" -o "${HOST}" = "controlplane02" ]; then
